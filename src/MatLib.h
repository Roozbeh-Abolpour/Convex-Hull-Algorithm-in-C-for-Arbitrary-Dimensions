...
void getrow2(Mat* A,Mat* B,int r){
	int k=0;
	for(int i=1;i<=A->col;i++){
		B->entries[k++]=A->entries[(r-1)*A->col+i-1];
	}
}
...
void scalermultiply2(Mat* M,Mat* B,double c){		
	int k=0;
	for(int i=0;i<M->row;i++){
		for(int j=0;j<M->col;j++){
			B->entries[k]=M->entries[k]*c;
...
Mat* exceptsubmat(Mat* A,int e){
	...
	for(int i=1;i<=l;i++){
		if(i!=e){
			B->entries[k]=A->entries[i-1];k++;
		}
	}
	return B;
}
void exceptsubmat2(Mat* A,Mat* B,int e){	
	int l=A->row;
	if(A->col>l){
		l=A->col;
	}
...
/*
void vconcat2(Mat* A,Mat* B,Mat* C){
	if(A==nullptr){
		C=copyvalue(B);
		return;
	}
...
Mat* null(Mat *A){
	...
	freemat(RRM);	
	scalermultiply2(R2,R2,-1);	
	Mat* T2=triinverse(R1);	
	Mat* X=multiply(T2,R2);	
	freemat(R1);
	freemat(T2);	
	Mat* I=eye(nn);
	Mat* N=vconcat(X,I);
...
Mat* sort(Mat* A){
	...
	while(i<l){
		int j=i-1;
		double temp=B->entries[i];
		while (j>=0&&temp<B->entries[j])
		{
...
void writemat(char* filename, Mat* A){
	int r=A->row;
	int c=A->col;	
	FILE* fid;
	fopen_s(&fid,filename,"wt");
	char ch=',';
	for(int i=1;i<=r;i++){
		for(int j=1;j<=c;j++){
			double x=A->entries[(i-1)*A->col+j-1];			
			fprintf(fid,"%f",x);
			if(j<c){
				fprintf(fid,",");
			}
...
double str2d(char* str,int l1,int l2){
	double sign=+1;
	if(str[l1]=='-'){
		sign=-1;
		l1+=1;
...
Mat* readmat(char* filename){	
	...
	int j=0;
	while(j<=fsize){
		if(string[j]==10||string[j]==','){
			double de=str2d(string,i,j-1);
			A->entries[k++]=de;
			i=j+1;
		}		
		j++;
